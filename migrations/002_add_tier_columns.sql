-- Function to add tier columns to all registration tables
CREATE OR REPLACE FUNCTION add_tier_columns_to_registration_tables()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    LOOP
        ...
        EXECUTE format('
            ...
            BEGIN
                BEGIN
                    ALTER TABLE %I ADD COLUMN tier_name TEXT;
                EXCEPTION
        ...
        -- Add tier_price column if it doesn't exist (this is redundant with price but more explicit)
        EXECUTE format('
            DO $$ 
            BEGIN
                BEGIN
                    ALTER TABLE %I ADD COLUMN tier_price NUMERIC;
                EXCEPTION
                    WHEN duplicate_column THEN NULL;
                END;
            END $$;
...
SELECT add_tier_columns_to_registration_tables();

-- Update the create_party_registration_table function to include the new columns
...
AS $$
BEGIN
  -- Create the registrations table with tier columns
  EXECUTE format('
